...
def javacpg_main(args):
    ...
    if args.encoding:    
        ...
        if not os.path.exists('../datasets/code4bench/perf_labels.txt'):
            logger.error('Perf labels not found! Please check the path: ../datasets/code4bench/perf_labels.txt')
            exit(1)
        os.system('cp {} {}'.format('../datasets/code4bench/perf_labels.txt', args.encode_path))
