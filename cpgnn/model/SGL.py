...
class SGL(object):
    ...
    pretrain_embedding:np.array) -> None:
        ...
        """ Build model for code performance degradation """
        with tf.device('/device:CPU:0'):
            if self.perf_train:
                self._build_perf_model()
                self._build_perf_loss()
    ...
    pretrain_embedding:np.array) -> None:
        ...
        # learning flags
        self.perf_train = args.perf_test_supervised
        self.classification_train = args.classification_test

        # statistics for dataset 
        self.model_end = args.model_end
    ...
    def _build_inputs(self) -> None:
        ...
        # input a pair of functions for code performance degradation prediction
        # f_e shape: [batch_size, entity_num]
        self.f1_perf = tf.placeholder(dtype=tf.int64, name='f1_perf',
            shape=[None, self.max_entity_func])
        self.f2_perf = tf.placeholder(dtype=tf.int64, name='f2_perf',
            shape=[None, self.max_entity_func])
        self.y_perf = tf.placeholder(dtype=tf.float32, name='y_perf',
            shape=[None, 1])
    ...
    def _build_weights(self) -> None:
        ...
        # weights for code performance degradation
        if self.perf_train:

            if self.model_end == "CONC":
                all_weight['w_perf'] = tf.Variable(
                    initial_value=initializer([self.inter_dim * 2, 1]), name='w_perf')
            else: all_weight['w_perf'] = tf.Variable(
                    initial_value=initializer([self.inter_dim , 1]), name='w_perf')
                
            all_weight['b_perf'] = tf.Variable(
                initial_value=initializer([1, 1]), name='b_perf')
        
            if self.model_end == "REGS":
                all_weight['w_perf2'] = tf.Variable(
                    initial_value=initializer([self.inter_dim , 1]), name='w_perf2')
                all_weight['b_perf2'] = tf.Variable(
                    initial_value=initializer([1, 1]), name='b_perf2')
                all_weight['w_perf3'] = tf.Variable(
                    initial_value=initializer([2 , 1]), name='w_perf3')
                all_weight['b_perf3'] = tf.Variable(
                    initial_value=initializer([1, 1]), name='b_perf3')
    ...
    def _euclidean_distance(self, nr1, nr2):
        perf_distance = tf.abs(nr1 - nr2)
        self.perf_diff = tf.matmul(perf_distance, self.weights['w_perf']) + self.weights['b_perf']
        return self.perf_diff

    def _difference(self, nr1, nr2):
        difference = nr1 - nr2
        self.perf_diff = tf.matmul(difference, self.weights['w_perf']) + self.weights['b_perf']
        return self.perf_diff

    def _concatenate(self, nr1, nr2):
        concatenated = tf.concat([nr1, nr2], axis=-1)
        self.perf_diff = tf.matmul(concatenated, self.weights['w_perf']) + self.weights['b_perf']
        return self.perf_diff

    def _compare_regressions(self, nr1, nr2):
        reg1 = tf.matmul(nr1, self.weights['w_perf']) + self.weights['b_perf']
        reg2 = tf.matmul(nr2, self.weights['w_perf2']) + self.weights['b_perf2']
        concatenated_regs = tf.concat([reg1, reg2], axis=-1)
        
        self.perf_diff = tf.matmul(concatenated_regs, self.weights['w_perf3']) + self.weights['b_perf3']
        return self.perf_diff 

    def _build_perf_model(self):
        """"""
        f1_perf_e = tf.nn.embedding_lookup(self.g_embedding, self.f1_perf)
        f2_perf_e = tf.nn.embedding_lookup(self.g_embedding, self.f2_perf)
        f1_perf_e = tf.nn.l2_normalize(f1_perf_e, 1)
        f2_perf_e = tf.nn.l2_normalize(f2_perf_e, 1)

        # max pooling entity embeddings in a function
        # f1_perf shape: [batch_size, inter_dim]
        f1_perf_p = tf.layers.average_pooling1d(
            inputs=f1_perf_e,
            pool_size=self.max_entity_func, strides=1,
            name='f1_perf_p')
        f1_perf_p = tf.reshape(f1_perf_p, [-1, self.inter_dim])

        f2_perf_p = tf.layers.average_pooling1d(
            inputs=f2_perf_e,
            pool_size=self.max_entity_func, strides=1,
            name='f2_perf_p')
        f2_perf_p = tf.reshape(f2_perf_p, [-1, self.inter_dim])
        
        """ unsupervised training """
        norm_f1_perf_e = tf.nn.l2_normalize(f1_perf_p, 1)
        norm_f2_perf_e = tf.nn.l2_normalize(f2_perf_p, 1)
        self.perf_prediction_unsupervised = tf.reduce_sum(tf.multiply(norm_f1_perf_e, norm_f2_perf_e), axis=1)

        """ supervised training """
        log.info("I am HERE")
        if self.model_end == "DIST":
            self.perf_prediction_supervised = self._euclidean_distance( norm_f1_perf_e, norm_f2_perf_e)
            log.info("PLEASE:")
            log.info(self.perf_diff)
        elif self.model_end == "DIFF":
            self.perf_prediction_supervised = self._difference( norm_f1_perf_e, norm_f2_perf_e)
        elif self.model_end == "CONC":
            self.perf_prediction_supervised = self._concatenate( norm_f1_perf_e, norm_f2_perf_e)
        elif self.model_end == "REGS":
            self.perf_prediction_supervised = self._compare_regressions( norm_f1_perf_e, norm_f2_perf_e)
        log.info("NOW HERE")

    def _build_perf_loss(self):
        """"""
        perf_diff = tf.nn.sigmoid_cross_entropy_with_logits(
            labels=self.y_perf,
            logits=self.perf_diff,
            name='perf_cross_entropy')

        self.perf_loss = tf.reduce_mean(perf_diff, name='perf_loss')

        # rep optimization
        if self.opt_type in ['Adam', 'adam']:
            self.perf_opt = tf.train.AdamOptimizer(learning_rate=self.lr).minimize(self.perf_loss)
        elif self.opt_type in ['SGD', 'sgd']:
            self.perf_opt = tf.train.GradientDescentOptimizer(learning_rate=self.lr).minimize(self.perf_loss)
        elif self.opt_type in ['AdaDelta']:
            self.perf_opt = tf.train.AdadeltaOptimizer(learning_rate=self.lr).minimize(self.perf_loss)
        else:
            log.error('Optimization is unknown')    
            exit(-1)

        log.info('Finish building loss for code performance degradation prediction')
    ...
    def train_perf(self, sess:tf.Session, feed_dict:dict) -> Tuple[float, float]:
        run_options = tf.RunOptions(report_tensor_allocations_upon_oom=True)
        return sess.run([self.perf_opt, self.perf_loss], feed_dict, options=run_options)

    def eval_classification(self, sess:tf.Session, feed_dict:dict) -> list:
        """"""
        return sess.run(self.classification_prediction, feed_dict)

    def eval_perf_unsupervised(self, sess:tf.Session, feed_dict:dict) -> list:
        """"""
        return sess.run(self.perf_prediction_unsupervised, feed_dict)

    def eval_perf_supervised(self, sess:tf.Session, feed_dict:dict) -> list:
        """"""
        return sess.run(self.perf_prediction_supervised, feed_dict)
