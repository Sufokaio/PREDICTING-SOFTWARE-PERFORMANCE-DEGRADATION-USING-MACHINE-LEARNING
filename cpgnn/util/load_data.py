...
class Data(object):
    def __init__(self, args:argparse.Namespace) -> None:
        ...
        # parse code perf data
        self.classification_num = int(args.classification_num)

        # batch_size for code perf (both supervised and unsupervised)
        self.batch_size_perf = args.batch_size_perf
        ...
        if args.perf_test_unsupervised or args.classification_test or args.cluster_test or args.perf_test_supervised:
            log.info('Parsing code perf degradation prediction dataset')
            if args.dataset.find('c4b') > -1:
                # load c4b dataset
                self.func_file = os.path.join(self.in_path, 'c4b_perf.txt')
                self.codelabel_file = os.path.join(self.in_path, 'perf_labels.txt')
                exist_dir(self.codelabel_file)
                self.max_entity_func, self.all_degradation_type = self._extract_c4b_code_perf()
    ...
    def _extract_c4b_code_perf(self):
        """ """
        # encode code perf data from Code4Bench dataset
        log.info('Extract functions from c4b dataset')
        func_dict = {}
        ...
        with open(self.func_file) as f:
            ...
            log.debug('The total number of functions: %d' % n_func)
        ...
        # 0: Type I, 1: True Negative
        all_degradation_type = [[] for _ in range(2)]

        # load ground-truth code performance degradation
        with open(self.codelabel_file) as f:
            n_perd_pair = int(f.readline().strip())  
            ...
            for line in f.readlines():
                ...
                all_degradation_type[type].append([func_1, func_2])

            assert(n_perd_pair == sum(len(degradation_type) for degradation_type in all_degradation_type))

            log.debug('Code Performance Degradation: [n_I, n_TN] = [%d, %d]'%(len(all_degradation_type[0]), len(all_degradation_type[1])))
            
        return max_entity_func, all_degradation_type
