...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--model_end', nargs='?', default='DIST',
                        help='type of end part of the model {DIST, DIFF, CONC, REGS}.')

    parser.add_argument('--save_model', default=False, action='store_true',
                        help='whether save SGL model parameters.')

    # code performance degradation
    parser.add_argument('--degradation_threshold', type=float, default=0.5,
                        help='cosine similarity code performance degradation prediction task')
    
    parser.add_argument('--batch_size_perf', type=int, default=128,
                        help='batch size for code performance degradation prediction')
    ...
    # code clone
    # unsupervised
    parser.add_argument('--perf_test_unsupervised', default=False, action='store_true',
                        help='whether to test code performance degradation prediction.')
    # supervised
    parser.add_argument('--perf_test_supervised', default=False, action='store_true',
                        help='whether to test code performance degradation prediction.')
    parser.add_argument('--perf_val_size', type=float, default=0.18,
                        help='Size of validation dataset for code performance degradation prediction')
    parser.add_argument('--perf_test_size', type=float, default=0.25,
                        help='Size of test dataset for code performance degradation prediction')
    ...
    if args.perf_test_unsupervised and args.perf_test_supervised:
        log.error('cannot train with both unsupervised and supervised code perf enabled')
        # exit(-1)

    if args.classification_test == False and args.perf_test_supervised == False:
        log.error('choose training either code representations or code classification or code perf')
        # exit(-1)
